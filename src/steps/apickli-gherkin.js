...
Given(/^I set the API domain to "([^"]*)"$/, function(urlKey) {
  this.apickli.domain = browser["world"].config.serverUrls.hasOwnProperty(urlKey) ? browser["world"].config.serverUrls[urlKey] : urlKey;
...
When(/^I GET (.*)$/, function(resource) {
  ...
  asyncWaitUntil(
    () => {
      pause(browser["world"].config.constants.API_TIMEOUT);
      return this.apickli.httpResponse.complete;
    },
    browser["world"].config.constants.API_TIMEOUT,
    browser["world"].config.constants.API_WAIT_CLICK,
    `Expected API to return response within ${browser["world"].config.constants.API_TIMEOUT}ms`
  );
});
...
Then(/^response body should be valid according to schema file (.*)$/, function(schemaFile) {
  ...
  asyncWaitUntil(
    ...
    browser["world"].config.constants.API_TIMEOUT,
    browser["world"].config.constants.API_WAIT_CLICK,
    `Expected API response to validate within ${browser["world"].config.constants.API_TIMEOUT}ms`
  );
...
Then(/^value of scenario variable (.*) should be (.*)$/, function(variableName, variableValue) {
  ...
  expect(assertion.success, `Value of variable ${variableName} is not equal to ${variableValue}`).to.be.true;
});
...
const asyncWaitUntil = function(fn, timeout, tickStep, error) {
  return new Promise(function(resolve, reject) {
    let elapsedTime = 0;

    (function waitForResultTick() {
      elapsedTime += tickStep;
      console.log(`Waiting for API response: ${elapsedTime}ms`);
      ...
      setTimeout(waitForResultTick, tickStep);
    })();
  });
};
